...
export default function AgentDetailPage() {
    ...
    const selectSession = async (rawSess: any, scopeOverride: 'mine' | 'all' = chatScope) => {
        ...
        try {
            ...
            if (writable) {
                ...
                for (const m of msgs) {
                    if (m.is_hidden) {
                        const firstLine = (m.content || '').split('\n').find((line: string) => { const trimmed = line.trim(); return trimmed && trimmed !== '---'; })?.replace(/^#\s*/, '').trim() || 'Skill loaded';
    ...
    interface ChatMsg { role: 'user' | 'assistant' | 'tool_call'; content: string; id?: string; fileName?: string; toolName?: string; toolCallId?: string; toolArgs?: any; toolStatus?: 'running' | 'done'; toolResult?: string; toolThinking?: string; thinking?: string; imageUrl?: string; timestamp?: string; isSkillIndicator?: boolean; hiddenContent?: string | null; }
...

                {
                    activeTab === 'chat' && (
                        ...
                        >
                            ...
                            <div className={`session-sidebar agent-chat-sidebar ${sessionListCollapsed ? 'collapsed' : ''}`} style={{ width: sessionListCollapsed ? '0px' : '220px', transition: 'width 0.2s ease', flexShrink: 0, minHeight: 0, borderRight: sessionListCollapsed ? 'none' : '1px solid var(--border-subtle)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
