...
async def get_public_base_url(db: AsyncSession, request: "Optional[Request]" = None) -> str:
    """Resolve the public base URL used for user-facing links."""
    from app.services.platform_service import platform_service
    return await platform_service.get_public_base_url(db, request=request)


async def build_password_reset_url(db: AsyncSession, raw_token: str, request: "Optional[Request]" = None) -> str:
    """Build the user-facing reset URL."""
    base_url = (get_settings().PUBLIC_BASE_URL or "").rstrip("/")
    if not base_url:
        base_url = (await get_public_base_url(db, request=request)).rstrip("/")
