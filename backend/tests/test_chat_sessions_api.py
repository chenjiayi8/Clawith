...
@pytest.mark.asyncio
async def test_all_scope_includes_group_sessions(monkeypatch):
    viewer_id = uuid.uuid4()
    agent_id = uuid.uuid4()
    now = datetime.now(UTC)
    ...
    assert sessions[0].group_name == "Sales war room"


@pytest.mark.asyncio
async def test_all_scope_includes_agent_peer_sessions(monkeypatch):
    viewer_id = uuid.uuid4()
    agent_id = uuid.uuid4()
    ...
    assert sessions[0].peer_agent_id == str(peer_agent_id)
    assert sessions[0].peer_agent_name == "Peer Agent"
